...
- [x] D1. 매처의 이름 유사도가 변별 토큰보다 단복수/하이픈에 좌우되는 문제 조사
      → 수정 완료. 문자-숫자 사이 구두점 접기(`DPP-4`≡`DPP4`) + 단순 복수형 접기 +
      범주 명사 단독 매칭 금지. `conceptset_overlap_eval.py`, 신규 테스트 11건.
- [x] D2. 6개 시험 전체에서 같은 부류의 오짝짓기가 몇 건인지 집계 (매크로 수치 신뢰도에 직결)
      → 232개 gold 중 **결정적 3건**(atc_fix/scale_fix 동일), gate_fix 2건.
      두 비교 arm에 같은 결함이 같이 있어 **델타는 거의 편향되지 않았다**.
- [x] D3. 수정 시 과거 arm 전부 재채점 필요 — 별도 세션에서 진행할 것
      → 3개 arm 재채점 완료(`scoped_*_rematch.json`). 매크로 recall
      gate 0.600→0.607 / scale 0.611→0.614 / atc 0.625→0.632.
      **scale→atc 델타 +0.017 → +0.018, 5/6 시험 동일 방향 — 결론 불변.**
