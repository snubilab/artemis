...
**발송 완료**: `output/circe_be/tte_circe_3studies_arms_20260912.zip` (81 KB, CARMELINA /
CAROLINA / EMPA-REG 팔별 6 개). 사용자가 2026-09-12 에 보냈다고 확인했고 대장
`docs/wiki/content/records/note-018.md` 행 3 에 기록했다. 6 개짜리도 같은 폴더에 있지만
기본이 아니다 — **기본 납품은 세 스터디**라는 상시 지시 (`e130541`).

`output/delivery/` 는 만들었다가 철회했다. `output/circe_be/` 가 2026-08-03 부터 이미
같은 역할을 하고 있었는데 못 알아보고 22 번째 폴더를 만든 것이었다.

## 납품 대장에서 바로잡은 것

행 1 (2026-08-31) 이 "Sent to the hospital" 로 적혀 있었으나 **보낸 적이 없다.** 산출물이
`circe_be/2026-08-31/` 에 존재한다는 사실에서 "보냈다" 를 추론해 쓴 행이었고, 2026-09-12 에
사용자 진술로 정정했다. 게이트·판정 칼럼은 `unknown` 이 아니라 **not applicable** 이다 —
판정은 병원 실행에서 오는데 그 실행이 없었다.

**납품은 사용자가 보냈다고 말할 때만 발송이다.** `circe_be/` 에 파일이 있다는 것은 준비됐다는
뜻이지 나갔다는 뜻이 아니다.

이 세션에서 같은 모양의 추론 오류를 세 번 했으니 적어둔다 — 전부 *존재* 에서 *상태* 를
추론한 것이다:

| 관찰한 사실 | 잘못 내린 결론 | 실제 |
| --- | --- | --- |
| 파일이 `circe_be/` 에 있다 | 보냈다 | 준비만 됨 |
| git 이 추적한다 | 여기가 자산 보관소 | 4 개 발송 중 1 개만 추적됨 |
| 코드에 경로 문자열이 있다 | 코드가 읽는다 | 주석 인용 (315 MB 가 주석 한 줄에 묶여 있었음) |

존재는 사실이고 상태는 주장이다. 상태는 실행해 보거나 물어봐야 안다.
...
- **위임 브리핑은 `docs/agent-briefing-facts.md`** 를 가리킬 것. 단, 이 세션에서 추가된 납품 규약
  ...
  `docs/agent-briefing-facts.md`는 이번 세션에 손대지 않았다 — **합침:** 조건부 요약 절을
  `docs/agent-briefing-facts.md`에 추가하고 `AGENTS.md` § WHERE A DELIVERY LIVES 를 전체
  내용의 근거로 남겼다 (기존 파일의 "요약 + AGENTS.md 교차참조" 관행을 그대로 따름).
