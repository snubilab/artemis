...
1. **리랭커가 우산 개념을 놓친다 — 여기가 다음 지렛대다.** LEADER `Type 1 diabetes`가
   교과서적 사례다: `435216`(+DESC, gold 130개 중 104개)이 top-15 안에 있는데 고르지
   않고, 대신 형제 개념 10개를 골라 recall 0.992 → 0.192가 됐다. 리랭커 프롬프트는 이미
   "넓은 상위 개념을 선호하라"고 지시하고 있으므로(critic 프롬프트 6·7번 지침),
   지시가 안 먹히는 이유부터 볼 것. **상수 재조정은 이미 해봤고 답이 아니다**(위 표).
2. **recall을 올리려면 리랭커를 봐야 한다.** 검색 랭킹은 크게 좋아졌는데
   (Measurement 커버리지 0.371 → 0.742) 최종 recall은 0.68 SE만큼만 움직였다.
   gold가 창 안에 있는데 안 뽑히는 사례가 회귀의 주된 형태다.
