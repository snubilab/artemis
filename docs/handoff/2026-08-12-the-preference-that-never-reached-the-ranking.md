...
**이 회귀들의 원인은 상수가 아니다 — 게인 스윕으로 확인했고, 첫 귀인은 틀렸다.**

처음엔 "이름 정확일치 −0.15가 강해져 좁은 개념이 넓은 gold를 밀어냈다"고 적었다. 게인
계수 하나를 0.0~1.5로 쓸어 측정하니 성립하지 않는다(조정 ARISTOTLE·CARMELINA·CAROLINA /
검증 EMPA-REG·LEADER·PLATO, 분할은 결과를 보기 전에 선언):

| 기준 | g=0 | g=0.15 | g=0.30 | g=1.0(현재) |
|---|---|---|---|---|
| Type 1 diabetes | 2/3 | 2/3 | 2/3 | 2/3 |
| bariatric surgery | 2/3 | 2/3 | 2/3 | 2/3 |
| anticoagulants | 0/6 | 0/6 | 0/6 | 0/6 |
| limb angioplasty | 1/10 | 1/10 | 0/10 | 0/10 |
| eGFR ×2 | 0/5 | 3/5 | 5/5 | 5/5 |

(숫자는 gold 멤버 중 top-15 안에 든 개수.)

LEADER `Type 1 diabetes`를 끝까지 파보면 정체가 분명하다. gold 130개는
`435216 Disorder due to type 1 diabetes mellitus`(+DESC, 혼자 104개)와
`201254`(25개)로 이뤄진다. 구 arm은 둘 다 골라 recall 0.992였다. 새 arm은 `201254`만
고르고 나머지 자리를 무관한 당뇨 형제 10개(Stimmler 증후군, 관해기 당뇨…)로 채워
closure 97 / 공유 25 / recall 0.192가 됐다. **그런데 `435216`은 모든 게인에서 top-15
안에 있다.** 검색은 올려줬고 리랭커가 안 골랐다.

- **게인은 만지지 말 것.** TUNE은 1.5가, HOLDOUT은 0.20이 최적이라고 말한다. HOLDOUT
  곡선은 0.327 → 0.345 → 0.315 → 0.323 → 0.330 → 0.333으로 지그재그이고, 게인 0.15~1.5
  구간의 전체 차이가 그 진폭(0.03) 안에 들어온다. 튜닝할 신호가 없다.
- 게인이 실제로 사주는 것은 Measurement뿐이다(커버리지 0.371 → 0.742, eGFR 0/5 → 5/5).
  0.30에서 포화한다.

**병목은 리랭커 선택으로 넘어갔다.** 후보는 올라오는데 고르는 쪽이 못 따라온다.

## 이어서 한 것 — ATC 임계값 (커밋 `7d0bb4b`, arm `atc_fix`)

리랭커를 더 파는 대신, 검색이 gold를 **top-60에도 못 올리는** 47개 기준을 봤다. 그중
DPP4·SGLT2·GLP-1은 gold가 RxNorm 성분 5~10개의 열거였다 — `"DPP4 inhibitors"`라는
문자열로는 `linagliptin`이 검색될 수 없다. 텍스트 유사도가 아니라 **약물 클래스 → 성분
열거**로 풀 문제이고, `drug_class_expander.py`가 이미 그 일을 한다.

그런데 안 돌고 있었다. `AGENT2_ATC_DISTANCE_THRESHOLD` 기본값 `0.4`가 의미상 정확한
매칭을 기각했다. 재매핑 로그의 31개 판정 전체:

- 통과 4건 (최대 거리 0.134), 기각 27건 (최소 0.453)
- 기각된 **0.453~0.735 구간은 전부 정답**: DPP-4 0.456, SGLT2 0.462/0.469/0.490,
  GLP-1 0.499/0.618/0.646, insulin 0.453/0.607/0.636, corticoid 0.537, anti-obesity 0.627
- 그 구간 유일한 오답은 `CYP3A inhibitors → Pi3K inhibitors` (0.760)

0.75가 정답 14건을 받고 오답 0건을 막는다. **0.4는 클러스터를 가른 게 아니라, 그 너머에
무엇이 떨어지는지 아무도 안 본 채 정해진 값이었다.** UMLS MRREL SQLite도 없어(로그 37회)
1번 전략은 죽어 있으므로 ATC가 유일한 경로다.

### 측정 (scale_fix → atc_fix, 공통 gold 집합 기준 짝지음)

| trial | recall | precision |
|---|---|---|
| ARISTOTLE | 0.662 → 0.670 (+0.008) | 0.405 → 0.399 (−0.005) |
| CARMELINA | 0.550 → 0.619 (+0.069) | 0.561 → 0.582 (+0.021) |
| CAROLINA | 0.579 → 0.600 (+0.021) | 0.594 → 0.585 (−0.009) |
| EMPA-REG | 0.442 → 0.443 (+0.002) | 0.439 → 0.439 (−0.000) |
| LEADER | 0.625 → 0.625 (0.000) | 0.604 → 0.602 (−0.002) |
| PLATO | 0.807 → 0.807 (0.000) | 0.734 → 0.729 (−0.005) |
| **MACRO** | **0.611 → 0.627 (+0.017)** | **0.556 → 0.556 (−0.000)** |

`recall +0.017 = 1.52 SE`, 5/6 개별 시험 개선. SE만 보면 여전히 결정적이지 않지만,
**대상 기준에서 인과가 직접 확인된다**는 점이 앞선 두 세션과 다르다:

| 기준 | recall | precision |
|---|---|---|
| SGLT2 inhibitors | 0.25 → **1.00** | 0.48 → 0.97 |
| GLP-1 RA (2개 시험) | 0.41 → **1.00** | 1.00 → 1.00 |
| DPP-4 inhibitor | 0.131 → **0.621** | 0.577 → **1.000** |
| [CKim] any insulin | 0.00 → 0.20 | 1.00 → 0.63 |

### 🔴 채점 하네스가 DPP4를 잘못 짝짓는다 — 매크로 수치를 깎아먹는다

`scoped_atc_fix.json`은 gold `[TROY intervention] DPP4 inhibitors`를 생성 집합
**`SGLT-2 inhibitors`**에 붙여 recall 0.07로 기록하고, 맞는 상대인 `id=9 'DPP-4
inhibitor'`(gliptin 7성분)를 `unmatched_generated`로 버린다. 직접 채점하면 0.621이다.

즉 위 매크로 `+0.017`은 **과소평가**다. 원인 추정: `DPP4 inhibitors` vs `DPP-4
inhibitor`의 하이픈·단수형 차이가 벌점을 받는 사이 복수형이 맞는 `SGLT-2 inhibitors`가
이름 유사도를 더 얻는다 — 변별 토큰(DPP vs SGLT)보다 어미가 세게 작용한다.
**고치려면 과거 arm을 전부 재채점해야 하므로 별도 세션에서 할 것.**

### 씨앗 실험 (4-arm, 재매핑 없음)

`top_n=3` 병목을 두 방향으로 시험했다. 74개 기준, 네 arm이 동일한 후보 15개를 채점.
...
top_n 효과는 두 조건에서 일관(+10.2 / +11.0pp)하고, 프롬프트 한 줄은 +0.9 / +1.7pp로
gold 1~2개 차이라 0과 구별되지 않는다. **커버리지를 프롬프트로 지시하는 접근은 기각.**
`top_n=5`는 이번 arm에 **넣지 않았다** — ATC와 섞으면 귀속이 불가능하다. 다음 arm 후보.
단 이 실험은 recall 쪽만 쟀고, 씨앗이 늘면 precision이 떨어질 수 있다.

## Next steps (ordered, concrete)

1. **채점 하네스의 짝짓기부터 고칠 것.** 위 DPP4 사례가 보여주듯 매처가 gold를 엉뚱한
   생성 집합에 붙이면 매크로 수치가 왜곡되고, 그 수치로 다음 결정을 내리게 된다.
   6개 시험 전체에서 같은 부류가 몇 건인지 먼저 집계할 것 — 이건 파이프라인 개선이
   아니라 **측정 도구 수리**이고, 지금 모든 판단이 이 도구 위에 서 있다.
2. **`top_n=3 → 5`가 다음 파이프라인 arm.** 씨앗 유지 62.7% → 72.9%가 오프라인으로
   확인됐다. precision 영향은 미측정이므로 재매핑으로 확인할 것.
3. **남은 47개 기준 중 약물 클래스가 아닌 부류.** `Type 2 diabetes mellitus`처럼 gold가
   포함 3개 + **제외 5개**의 큐레이션 표현식인 것들은 개념 선택이 아니라 표현식 구성
   문제다. ATC 같은 단일 지렛대가 없다.
4. **리랭커가 우산 개념을 놓치는 문제**는 남아 있다. LEADER `Type 1 diabetes`에서
   `435216`(gold 130개 중 104개)이 모든 게인에서 top-15 안에 있는데 안 뽑힌다.
   **상수 재조정은 해봤고 답이 아니다**(위 게인 스윕 표).
