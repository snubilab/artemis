...
def main() -> None:
    ...
    # Generated by scripts/build_value_taxonomy.py; every count in it is a corpus query.
    tax_path = OUT.parents[1] / "docs" / "daily_notes" / "tte_value_taxonomy.json"
    ...
    html = html.replace("/*__TAX__*/", json.dumps(tax, ensure_ascii=False))
...
.taxex{margin-top:8px;padding:9px 11px;background:var(--surface-2);border:1px solid var(--line);border-radius:8px;font-size:12.5px}
.taxex .pill{margin-left:6px}
...
<nav class="tabs" role="tablist">
  ...
  <span class="tabgroup" role="presentation" data-glabel="Record">
    ...
    <button class="tab" id="tab-tax-btn" role="tab" aria-selected="false" aria-controls="tab-tax" data-tab="tax">분류 체계</button>
  </span>
</nav>
...
<div id="tab-tax" class="tabpanel" role="tabpanel" aria-labelledby="tab-tax-btn" hidden>
  <div class="sec-head" style="margin-bottom:10px">
    <span class="eyebrow">Value Threshold Taxonomy</span>
    <h2 style="text-transform:none;font-size:20px;margin:4px 0 0">분류 체계</h2>
    <div class="sub">eligibility 임계값을 <b>무엇을 재는 숫자인지</b>로 갈라 Circe 목적지를 정하는 stage 2 분류 체계. 클래스·연산자·표기 세 축 모두 <span class="mono">tests/fixtures/value_constraint_corpus.yaml</span> 114건에서 도출했고, <b>모든 수치는 코퍼스 질의 결과</b>입니다. 정의문은 분류 프롬프트에 그대로 들어가는 원문이라 영문입니다. <span class="mono">python scripts/build_value_taxonomy.py</span>로 재생성, <span class="mono">--prompt</span>로 프롬프트를 렌더합니다.</div>
  </div>
  <div id="tax"></div>
</div><!-- /tab-tax -->
...
<script id="taxdata" type="application/json">/*__TAX__*/</script>
...
const TAX=JSON.parse(document.getElementById('taxdata').textContent);
...
// The taxonomy artifact is generated from the corpus, so this renderer walks its real
// structure (classes / op_table / notation / unresolved / limits) rather than a
// display-shaped copy — a second shape would reintroduce the drift it exists to prevent.
// English definition text is deliberate: it is the verbatim classifier-prompt wording.
function taxView(listId, data){
  const el=document.getElementById(listId); if(!el||!data||!Array.isArray(data.classes)) return;
  const meta=data.meta||{};
  const tbl=(head,rows)=>`<table class="datatable"><thead><tr>${head.map(h=>`<th>${mdN(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr>${r.map(c=>`<td>${c}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
  const hd=t=>`<div style="color:var(--accent);font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.06em;margin-top:12px">${mdN(t)}</div>`;
  const ul=items=>`<ul style="margin:5px 0 0;padding-left:18px;font-size:14px;color:var(--ink-2)">${items.map(i=>`<li style="margin:3px 0">${mdN(i)}</li>`).join('')}</ul>`;
  const verdictCls=v=>/^correct/.test(v||'')?'ok':'warn';
  const ex=e=>`<div class="taxex"><span class="mono" style="color:var(--ink)"><b>${esc(e.threshold_phrase)}</b></span>
    <span class="pill">${esc(e.id)}</span><span class="pill">${esc(e.source)}</span><span class="pill">kind: ${esc(e.kind)}</span>
    <div style="font-size:12px;color:var(--ink-3);margin-top:5px">${esc(e.source_text)}</div></div>`;

  const tiles=[[meta.corpus_entries,'코퍼스 엔트리'],[meta.corpus_sources,'출처 프로토콜·논문'],
    [data.classes.length,'클래스'],[(data.op_table||[]).filter(r=>r.corpus_count).length,'코퍼스로 확인된 연산자 표기']]
    .map(([n,l])=>`<div class="tile"><div class="n">${esc(n)}</div><div class="l">${esc(l)}</div></div>`).join('');

  const rules=`<div class="card" style="margin-bottom:16px">
    <div class="chart-t" style="margin:0">분류기 하드 룰 · 프롬프트 계약</div>
    <div class="chart-s">아래 문장은 <span class="mono">--prompt</span>가 그대로 출력하는 원문입니다. 프롬프트는 이 파일의 순수 함수라 따로 관리하지 않습니다.</div>
    ${ul(meta.hard_rules||[])}
    ${hd('출력 스키마')}<pre class="mono" style="white-space:pre-wrap;background:var(--surface-2);border:1px solid var(--line);border-radius:8px;padding:10px;margin:6px 0 0;font-size:11.5px;color:var(--ink-2)">${esc(JSON.stringify(meta.output_schema||{},null,2))}</pre>
    ${hd('왜 정규식이 아니라 LLM인가')}<div style="font-size:13.5px;color:var(--ink-2);margin-top:5px">${mdN(meta.why_llm||'')}</div></div>`;

  const classes=data.classes.map(c=>`<div class="card" style="margin-bottom:16px">
    <div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;flex-wrap:wrap">
      <div class="chart-t" style="margin:0">${esc(c.id)} <span style="color:var(--ink-3);font-weight:500">· ${esc(c.label_ko)}</span></div>
      <span class="refverdict ${c.corpus_count?'ok':'bad'}">n = ${c.corpus_count} · ${(c.corpus_share*100).toFixed(1)}%</span></div>
    <div class="note-key">${mdN(c.summary_ko||'')}</div>
    <div class="kv" style="margin-top:11px">
      <div class="k">정의 (프롬프트 원문)</div><div class="v">${mdN(c.definition)}</div>
      <div class="k">Circe 목적지</div><div class="v">${mdN(c.circe_destination)}</div>
      <div class="k">stage 3</div><div class="v mono">${esc(c.stage3)}</div>
      <div class="k">초안 대비</div><div class="v"><span class="tag ${verdictCls(c.draft_verdict)}">${esc(c.draft_verdict)}</span></div>
      ${c.naming_caveat?`<div class="k">이름 주의</div><div class="v">${mdN(c.naming_caveat)}</div>`:''}
    </div>
    ...
    <details><summary>코퍼스 원문 예시 ${c.examples.length}건</summary>${c.examples.map(ex).join('')}</details>
  </div>`).join('');

  const ops=`<div class="card" style="margin-bottom:16px">
    <div class="chart-t" style="margin:0">연산자 표기 → Circe op</div>
    <div class="chart-s">ClinicalTrials.gov 이스케이프·유니코드·단어형을 모두 포함. n은 코퍼스 실측이며 0은 표기만 등록되고 코퍼스에 없다는 뜻입니다.</div>
    ${tbl(['표기','Circe op','n','예시 엔트리','비고'],(data.op_table||[]).map(r=>[
      `<span class="mono"><b>${esc(r.surface)}</b></span>`,
  ...
  const notation=`<div class="card" style="margin-bottom:16px">
    ...
    <div class="chart-s">stage 3가 흡수해야 하는 표면 변이. 각 항목의 <span class="mono">질의</span>가 그 수치를 만든 코퍼스 질의입니다.</div>
    ${(data.notation||[]).map(s=>`<div style="margin-top:14px">
      ${hd(s.label_ko+' · '+s.id)}
      <div style="font-size:13.5px;color:var(--ink-2);margin-top:4px">${mdN(s.what)}</div>
      <div class="kv" style="margin-top:6px">
        <div class="k">코퍼스</div><div class="v"><b class="num">${s.count}</b> / ${s.over}</div>
        <div class="k">질의</div><div class="v mono" style="font-size:11.5px">${esc(s.query)}</div>
        <div class="k">규칙</div><div class="v">${mdN(s.rule)}</div>
        ${s.note?`<div class="k">메모</div><div class="v">${mdN(s.note)}</div>`:''}
      </div>
      ${s.variants&&s.variants.length?`<details><summary>변이 ${s.variants.length}종</summary>${tbl(['형태','n','비고'],s.variants.map(v=>[
        `<span class="mono">${esc(v.form)}</span>`,`<span class="num">${v.n}</span>`,
        esc([v.id,v.note].filter(Boolean).join(' · '))]))}</details>`:''}
    </div>`).join('')}</div>`;

  const unresolved=`<div class="card" style="margin-bottom:16px">
    <div class="chart-t" style="margin:0">단일 클래스에 저항하는 엔트리</div>
    <div class="chart-s">클래스가 하나로 안 정해지는 항목과, 그래서 어떻게 처리했는지.</div>
    ${tbl(['엔트리','배정','충돌','처리'],(data.unresolved||[]).map(u=>[
      `<span class="mono"><b>${esc(u.entry_id)}</b></span>`,`<span class="tag warn">${esc(u.class_assigned)}</span>`,
      mdN(u.tension), mdN(u.disposition)]))}</div>`;

  const limits=`<div class="card" style="margin-bottom:16px">
    <div class="chart-t" style="margin:0">이 체계가 분류하지 못하는 것</div>
    <div class="chart-s">클래스를 늘려서 해결되지 않는 한계. 코퍼스 근거와 실패 시 결과를 함께 적었습니다.</div>
    ${tbl(['한계','내용','코퍼스 근거','결과'],(data.limits||[]).map(l=>[
      `<span class="mono"><b>${esc(l.id)}</b></span>`,mdN(l.what),mdN(l.corpus_evidence),mdN(l.consequence)]))}</div>`;

  el.innerHTML=`<div class="tiles">${tiles}</div>`+rules+classes+ops+notation+unresolved+limits;
}
